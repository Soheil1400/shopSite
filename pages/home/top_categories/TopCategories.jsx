...
export default function Category() {
    ...
    const settings = {
        infinite: false, slidesToShow: 3, slidesToScroll: 1, swipeToSlide: true, responsive: [{
    ...
    return (
        <Box sx={{maxWidth: "1200px", width: '98%',}}>
            <Title title={'Top Categories'} icon={<DashboardIcon/>}/>
            <Slider {...settings}>
                {topCategory.map(item => (<CartBox key={item.id} component="span">
                    <TitlesBox>
                        <Chip
                            size={"small"}
